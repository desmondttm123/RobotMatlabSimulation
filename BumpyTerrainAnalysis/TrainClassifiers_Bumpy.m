%% TrainClassifiers_Bumpy.m - Train ML models on Bumpy Terrain RSSI data
% Same pipeline as TrainClassifiers.m but reads from local Results/ folder
% 6 classes: {DrySand, GrassySoil, Rocks} x {Object, NoObject}
% Features: 128 RSSI + 27 statistical = 155 total
% Models: SVM, MLP, Ensemble, Bagged Trees, KNN, Two-Stage, RUSBoost
...
%% Determine object/no-object labels
...
%% Build feature matrix and labels (155 features)
nFeatures_RSSI = 128;
nFeatures_Stats = 27;
...
for ti = 1:nTerrains
    ...
    s1_file = fullfile(output_dir, sprintf('RSSI_Sensor1_%s.csv', terrain_name));
    s2_file = fullfile(output_dir, sprintf('RSSI_Sensor2_%s.csv', terrain_name));
    
    s1_raw = readmatrix(s1_file);
    s2_raw = readmatrix(s2_file);
    
    s1_low = s1_raw(:, 2:nRX+1);
    s1_high = s1_raw(:, nRX+2:2*nRX+1);
    s2_low = s2_raw(:, 2:nRX+1);
    s2_high = s2_raw(:, nRX+2:2*nRX+1);
...
T.Label = Y_labels;
writetable(T, fullfile(output_dir, 'TrainingData.csv'));
fprintf('Saved: %s\n', fullfile(output_dir, 'TrainingData.csv'));
...
%% Compute metrics
fprintf('\n========================================\n');
fprintf('    BUMPY TERRAIN CLASSIFICATION RESULTS\n');
fprintf('========================================\n');
...
sgtitle('Bumpy Terrain - Classification Results');
savefig(gcf, fullfile(output_dir, 'ConfusionMatrices.fig'));
...
ylabel('Score'); title('Bumpy Terrain - Classifier Comparison');
...
savefig(gcf, fullfile(output_dir, 'MetricsComparison.fig'));
exportgraphics(gcf, fullfile(output_dir, 'MetricsComparison.png'), 'Resolution', 150);
close(gcf);

%% Save models
save(fullfile(output_dir, 'TrainedModels.mat'), 'results', 'mdl_svm', 'mdl_mlp', ...
    'mdl_ens', 'mdl_bag', 'mdl_knn', 'mdl_rus', 'feat_names', 'train_cats');
fprintf('\nModels saved to: %s\n', fullfile(output_dir, 'TrainedModels.mat'));
fprintf('\nDone.\n');
